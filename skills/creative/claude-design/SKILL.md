...
## Using Design Skills for Existing Repo Implementation

When the task is redesigning an existing frontend repo (not creating a standalone artifact), use design skills as the *active implementation pipeline*, not just reference material:

1. **Load the relevant design skill(s)** — `popular-web-designs` for the target design system's tokens, `claude-design` for the process
2. **Inspect the existing codebase** — read the actual source files (theme files, token files, global stylesheets, component files) BEFORE designing anything. The skill's process says "look for source context" — in a repo, that means the actual code
3. **Extract current values** — grep the actual color tokens, font stacks, radius values, border treatments from the existing implementation
4. **Create a design token reference file** — write the extracted values + target design system values as a `linear-tokens.ts` (or similar) under the repo's theme directory. This documents the gap and the target
5. **Delegate the implementation** — write a delegate_task prompt that includes: the exact current values, the target design system values, the files to modify, and the design skill's guidance as context. The subagent should make the changes, then verify TypeScript compiles clean
6. **Verify locally** — after the subagent finishes, run type checks and check the actual changed files match intent

Critical rule: **Do not manually edit design token files if a subagent can do it more accurately** with the full token spec in context. The design skill is the implementation knowledge, not just background reading.

Common mistake: reading the design skill for vocabulary, then manually applying it. The skill should be the vehicle for implementation via delegation, not just a glossary.
...
- **JavaScript `const` initialization order:** Any function that uses a `const`-declared variable before that declaration line is in the code will throw a `ReferenceError: Cannot access 'X' before initialization`. In chained initialization patterns (e.g., `Graph = ForceGraph(...); Graph.configChain(...); btn.onclick = updateVisibility`), the `const Graph` declaration MUST appear before any `onclick` handler that calls `updateVisibility()`. Use `let Graph` or rearrange so all uses follow the declaration. This is a temporal dead zone (TDZ) error, not a runtime error — it fires immediately on page load.
- **Vendor-bundle external JS libraries:** When using CDN-hosted scripts (e.g., `unpkg.com`, `cdn.jsdelivr.net`), download the file and serve it locally in a `vendor/` directory within the same repo/deploy path. CDN-hosted minified JS can silently fail, serve stale cached versions, or return non-functional content. Local vendoring is the only reliable approach for self-contained artifacts. Download with `curl -s https://cdn.url/file.min.js -o vendor/file.min.js`, update the `<script src>` to `vendor/file.min.js`, and commit the vendor file.
- **ForceGraph library API pattern:** `ForceGraph(element)(data)` (function-call chaining) does NOT work — the container element fails to attach with `appendChild is not a function`. The correct pattern matching official examples is `new ForceGraph(element).graphData(data)` — use the `new` keyword and chain `.graphData()` instead of passing data as a second function call. Always verify against the [official basic example](https://vasturiano.github.io/force-graph/example/basic/index.html) source.
- **Test the deployed artifact, not just the local file:** A locally-correct HTML file can break on GitHub Pages due to path casing, case-sensitive file references, case-sensitive link targets, or missing `index.html` default. After deploying, always verify: (1) the page loads at the deployed URL with HTTP 200, (2) key resources (JS, CSS, images) are accessible, (3) no console errors. Do this before telling the user "it's live."
