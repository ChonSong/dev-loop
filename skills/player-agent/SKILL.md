...
1. **Read `last_run`** from the master checkpoint at `./master-checkpoint.json`
...
- **Coach verdict is now JSON** — after each review, the Coach outputs a structured JSON verdict following `coach-agent/references/verdict-schema.json` and writes the verdict into the project `.checkpoint.json` under `coach_review.verdict`. Valid values: `"APPROVE"`, `"FIX"`, `"REVERT"`. Do NOT manually set coach status — the Coach's JSON verdict is the single source of truth. If the verdict is `REVERT`, your next tick's first action is to revert the last commit and investigate.
