...
## Default Date Values in SQL Server

SQL Server `datetime`/`datetimeoffset` columns may contain `0001-01-01 00:00:00` as a default/unset sentinel. This is **not** NULL — it passes `IS NOT NULL` checks.

**Always filter in CTEs/HAVING:**
```sql
-- For aggregated end dates:
HAVING MAX(rlrj.LockOffDate) > '2000-01-01'

-- For nullable end-date columns:
WHERE (ri.RemovalDate IS NULL OR ri.RemovalDate > '2000-01-01')

-- Also validate start <= end:
AND MIN(rlrj.LockOnDate) <= MAX(rlrj.LockOffDate)
```

**Threshold:** Use `> '2000-01-01'` — safely before any real operational data but catches all `0001-01-01` defaults.

## Isolation Date Columns (RFIIsolations)

The `RFIIsolations` table has explicit applied/removal dates:
- `AppliedDate` — when the isolation was applied (NOT NULL for active isolations)
- `RemovalDate` — when removed (NULL = still active)
- `RFIIsolationState` — state enum (3 = normal, other values for special states)

For Gantt charts showing isolation duration: `AppliedDate → RemovalDate` (or `→ JobEnd` if still active).
