#!/usr/bin/env python3
"""Generate a repo catalog from GitHub owned + starred repos.

Usage: python3 generate-catalog.py /path/to/catalog-repo-dir

Requirements: gh CLI authenticated, available on PATH.
No external Python dependencies (no PyYAML).

Outputs:
  owned/*.md   — per-repo files with YAML frontmatter
  starred/*.md — per-repo files with YAML frontmatter
  README.md    — index with stats, tables, tag index, combinatorial pairings
"""
import json
import os
import subprocess
...
def extract_tags(desc: str, readme: str = "") -> list:
    ...
    keyword_map = {
        ...
        "docker": "docker", "infrastructure": "infrastructure", "transmut": "transpilation",
        "circuit breaker": "reliability", "energy": "energy", "carbon": "sustainability",
        "benchmark": "benchmarking", "poker": "gaming", "starcraft": "gaming",
...
def main():
    ...
    for r in owned_data:
        ...
        entry = {
            ...
            "description": desc or "No description", "type": type_,
            "status": status, "language": lang or "other",
            "size_kb": size, "stars": stars, "last_pushed": pushed,
    ...
    for r in starred_filtered:
        ...
        entry = {
            ...
            "url": f"https://github.com/{name}",
            "description": desc or "No description", "type": type_,
            "status": status, "language": lang, "size_kb": size,
