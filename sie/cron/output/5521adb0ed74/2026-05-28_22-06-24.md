...
✅ **All 6 edge case areas polished and verified.** Here's the summary:

| Task | Status | Verification |
|------|--------|-------------|
| **A. PLO4 Wheel/Flush** | ✅ | Wheel A-2-3-4-5 detected across all board/hole splits; flush works with 4-board-suit + 2-in-hand; 1-heart hand correctly can't make flush vs 4-heart board. 10 new tests. |
| **B. PLO5 100 Combos** | ✅ | C(5,2)=10 × C(5,3)=10 = 100 combos confirmed. Algorithm iterates exactly 100 evaluations. 5 new tests. |
| **C. Omaha Hi/Lo Paired Board** | ✅ | A-2-3-4-4 board with 5-6 in hole → valid low (selects A-2-3 from board, avoids pair). Same board with 9-T-K-Q → no low (no qualifying hole cards). Verified 9 disqualifies, 8-or-better boundary. 6 new tests. |
| **D. Shortdeck Flush > FH > Straight** | ✅ | Flush (type 7) beats FH (type 6) beats Straight (type 5). A-6-7-8-9 wheel detected correctly. Best-5 from 7 cards correctly picks flush over FH. 8 new tests. |
| **E. Double Board Deck Overlap** | ✅ | Sequential deck slicing from shuffled deck guarantees no overlap. `_fill_board` uses `pop()` so boards never share cards. Partial MC and full exact modes both clean. 7 new tests. |
| **F. Bomb Pot Action Space** | ✅ | `BombPotState.get_action_space()` exists. STRADDLE_ROUND: straddle/call/raise/check (no fold). FLOP/TURN/RIVER: call/raise/check/fold. SHOWDOWN: empty. Action space transitions correctly through phases. 9 new tests. |

**185 tests total** (140 original + 45 new), **all passing**. Committed as `fix(variants): polish edge cases` and pushed to `origin main`.
