...
### Core Library (`packages/poker-core/src/gto_poker/`)
| File | Description |
|------|-------------|
| `plo4.py` | PLO4Evaluator using HenryRLee/PokerHandEvaluator (9-card: 4 hole + 5 board, picks best 2+3), PLO4Equity with Monte Carlo, rank-to-percentile |
| `plo4_range.py` | PLO4RangeParser for PLO4 notation (AAKK ds, TJs, A234 wraparounds) |
| `__init__.py` | Updated exports |

### API Endpoints (`apps/api/routers/`)
| File | Endpoints |
|------|----------|
| `plo4_equity.py` | `POST /plo4/equity/calculate`, `POST /plo4/equity/hand-rank`, `POST /plo4/equity/range-vs-range` |
| `plo4_ranges.py` | `POST /plo4/ranges/parse`, `POST /plo4/ranges/expand`, `GET /plo4/ranges/push-fold-chart/{position}`, `GET /plo4/ranges/hand-rankings` |

### Frontend (`apps/web/src/app/plo/page.tsx`)
- PLO4 equity calculator UI with hand inputs, board input, Monte Carlo samples slider
- Visual equity bar, strength classification, `/plo` nav link in Header

### Tests (`packages/poker-core/tests/test_plo4.py`)
- **18 tests passing**: evaluator (flush, straight, quads), range parser (AAKK ds, TJs, A234), equity, integration

### Key Technical Details- Uses `phevaluator` (Python port of HenryRLee/PokerHandEvaluator) via `/tmp/PokerHandEvaluator/python`
- PLO4 evaluation: C(4,2) × C(5,3) = 60 combos, picks best 5-card hand
- Lower rank = stronger hand (1 = royal flush, 7462 = high card)

**Commit:** `5899835 feat(plo4): add PLO4 support with HenryRLee/PokerHandEvaluator`
