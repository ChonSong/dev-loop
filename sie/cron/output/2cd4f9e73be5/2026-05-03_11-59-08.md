# Cron Job: agent-os morning deploy

**Job ID:** 2cd4f9e73be5
**Run Time:** 2026-05-03 11:59:08
**Schedule:** 0 21 * * *
...
Deploy the latest agent-os image on the server and verify it's healthy.

Context:
- Server: 192.168.1.117 (LAN)
- Docker compose path: /opt/data/hermes-sync/projects/agent-os/docker-compose.yml
- Image: ghcr.io/chonsong/agent-os:latest (built by GitHub Actions on main branch push)
- SSH user: root
- Docker socket is available on the host at /var/run/docker.sock

Steps:
1. SSH to the server (use subprocess with key from /tmp/github_actions_deploy if it exists, otherwise skip SSH and run docker commands directly)
2. Pull latest: docker pull ghcr.io/chonsong/agent-os:latest
3. Restart: cd /opt/data/hermes-sync/projects/agent-os && docker compose up -d
4. Wait 15 seconds for container to start
5. Health check: curl -sf http://localhost:9120/health
6. Verify docker ps shows agent-os running
7. Check nanobot serve is responding: curl -sf http://localhost:8900/health
8. Check AIE log has events: cat /opt/data/aie-logs/agent-events.jsonl | tail -3
9. Report: image SHA, container status, health check result, any errors

Use Python subprocess for all commands. Report all output.

## Response

**Deployment Verification Report — 2026-05-03 11:54**

**Environment:** This session is running inside the hermes-agent container on 192.168.1.117. However, critical access is not available:

| Check | Result |
|-------|--------|
| Docker socket | Not mounted — `/var/run/docker.sock` does not exist in this container |
| Docker TCP port | Closed on all standard ports (2375/2376/2377) |
| SSH daemon | Not running on localhost or 127.0.0.1 |
| `curl` | Not available in this container |
| `systemctl` | Not available |
| `ping` | Not available |

**Docker Compose file confirmed present** at `/opt/data/hermes-sync/projects/agent-os/docker-compose.yml`:
```yaml
services:
  agent-os:
    ...
    ports: 9120, 8900
    healthcheck: curl -sf http://localhost:9120/health
```

**Health endpoints unreachable** (services not responding on localhost:9120 or :8900) — either agent-os is not running, or it's running on the host but network-isolated from this container.

**Cannot complete deployment verification because:**
1. Docker socket not mounted — cannot `docker pull` or `docker compose up -d` from inside the hermes-agent container
2. SSH access to host not available — cannot reach host Docker from here

**Recommendation:** To manage agent-os from this hermes-agent instance, the Docker socket must be mounted into this container (add `-v /var/run/docker.sock:/var/run/docker.sock` to the hermes container run command on the host), or SSH must be enabled on the host with keys configured.
