# Cron Job: GTO Phase 3 — Strategy View + Final Polish (FAILED)

**Job ID:** 871aa0f60d73
**Run Time:** 2026-06-05 14:45:53
**Schedule:** once at 2026-06-05 14:45

## Prompt

[IMPORTANT: The following skill(s) were listed for this job but could not be found and were skipped: test-driven-development. Start your response with a brief notice so the user is aware, e.g.: '⚠️ Skill(s) not found and skipped: test-driven-development']
[IMPORTANT: The user has invoked the "creative/claude-design" skill, indicating they want you to follow its instructions. The full skill content is loaded below.]
...
PHASE 3 — Add GTO strategy view and final page polish.

Context: GTO Wizard Clone at /workspace/gto-wizard-clone.
API on port 8002.

## Task A: Create Strategy View Component

Create /workspace/gto-wizard-clone/apps/web/src/components/strategy/StrategyMatrix.tsx:
- 13x13 grid showing GTO strategy (bet/check/fold frequencies)
- Each cell color-coded by primary action:
  - Bet/raise: blue (#3b82f6) with opacity = frequency
  - Check/call: green (#22c55e) with opacity = frequency  
  - Fold: red (#ef4444) with opacity = frequency
- Cell shows primary action icon (▲ for bet, ● for check, ✕ for fold)
- Click to expand cell showing exact frequencies
- Legend showing action meanings

## Task B: Create Solver Progress Display

Create /workspace/gto-wizard-clone/apps/web/src/app/strategy/page.tsx OR update strategy page:
- URL input to load a stored strategy
- Display strategy metadata (game type, stack depth, street)
- Show the strategy matrix with action frequencies
- Allow switching between hero/villain perspective

## Task C: Polish All Pages

Update each main page to use consistent GTO Wizard styling:

1. /workspace/gto-wizard-clone/apps/web/src/components/Header.tsx — ensure nav links work (equity, icm, train, courses, spots, analyze, strategies)
2. /workspace/gto-wizard-clone/apps/web/src/app/icm/page.tsx — verify ICM calculator renders with working API calls
3. /workspace/gto-wizard-clone/apps/web/src/app/train/page.tsx — verify training page renders
4. /workspace/gto-wizard-clone/apps/web/src/app/page.tsx — make home page redirect to equity calculator

## Task D: Build and Verify

1. cd /workspace/gto-wizard-clone/apps/web && rm -rf .next && NEXT_PUBLIC_API_URL=http://localhost:8002 /home/hermeswebui/.hermes/home/.local/bin/node /workspace/gto-wizard-clone/node_modules/next/dist/bin/next build 2>&1 | tail -5
2. Verify at least /equity and /icm pages serve: curl -s http://localhost:8555/equity | head -c 100 && echo OK
3. git add -A && git commit -m "feat(strategy): strategy view + final polish" && git push origin main 2>&1 | tail -3
4. Start: fuser -k 8555/tcp 2>/dev/null; sleep 1; cd /workspace/gto-wizard-clone/apps/web && NEXT_PUBLIC_API_URL=http://localhost:8002 /home/hermeswebui/.hermes/home/.local/bin/node /workspace/gto-wizard-clone/node_modules/next/dist/bin/next start -p 8555 &
