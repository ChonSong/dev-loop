# Cron Job: phase-engine: hermes-web-computer completion

**Job ID:** 9ed5a5ab3dc4
**Run Time:** 2026-05-13 05:18:27
**Schedule:** every 30m
...
prompt: |
  ...
  2. If all phases complete → report success and stop
  3. If next phase is pending → execute it completely
  4. After execution:
     a. Verify build/tests pass
     b. git add + commit + push
     c. Write checkpoint to CHECKPOINTS/phase-{id}-complete.md
     d. Update PHASE_TRACKER.json with completion status
  5. Report results
...
16. **Web search IS enabled by default** — The `web_search` tool is in `_HERMES_CORE_TOOLS` in `toolsets.py`. Don't assume it's disabled — test it with a simple query. API keys for Brave/EXA are in `.env`.
17. **`/goal` command exists but is single-session only** — It's implemented in `/opt/hermes/hermes_cli/goals.py` (the "Ralph loop"). It auto-continues across turns within ONE session using a judge model to evaluate completion. It does NOT survive session restarts, context compaction, or timeouts. For multi-phase/large projects, use the Persistent Phase Engine (recurring cron + PHASE_TRACKER.json), not `/goal`.
...
```bash
# Build frontend first (Go server serves static files from frontend/dist)
...
PORT=3005 go run ./cmd/server/   # or: go build -o server ./cmd/server/ && PORT=3005 ./server
...
### ❌ Not Started
- **hermes-computer rebuild cron** (`ecb3846b907b`) is failing — `npm run build` from repo root fails because the frontend build is `frontend/` not root. Fix: change rebuild command to `cd frontend && npm run build`.
...
**Phase 1 complete.** `tool.execute` is now wired to the Hermes Agent API.

**Summary:**
- **Commit:** `a6d29f4` — `feat: wire tool.execute to Hermes Agent API`
- **What changed:** Replaced the `log.Printf` TODO stub with a full `handleToolExecute` function that:
  - Parses `{session_id, tool_name, arguments}` from the WS message
  - Calls `POST http://localhost:8642/v1/chat/completions` with tool forcing
  - Sends `tool.result` or `tool.error` events back via WebSocket
- **Build:** ✅ `go build` success
- **Tests:** ✅ All tests pass (ws package: 1.081s)
- **Pushed:** ✅ `main`

Phase tracker updated — Phase 2 (`apps-protocol`) is next.
