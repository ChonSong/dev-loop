...
Dependencies: None (no PyYAML). Parses YAML frontmatter manually.
Fetches data via `gh` CLI — must be authenticated.
"""
import json
import os
import subprocess
...
def extract_tags(desc: str, readme: str = "") -> list:
    ...
    tags = set()
    keyword_map = {
        "dashboard": "dashboard", "agent": "agent", "ai": "ai", "llm": "llm",
        "cli": "cli", "web": "web-app", "api": "api", "bot": "bot",
        "monitor": "monitoring", "backup": "backup", "sync": "sync",
        "docker": "docker", "infrastructure": "infrastructure",
        "circuit breaker": "reliability", "energy": "energy",
        "carbon": "sustainability", "benchmark": "benchmarking",
        "poker": "gaming", "starcraft": "gaming",
        "dotfiles": "dotfiles", "landing": "website", "mobile": "mobile",
        "voice": "voice", "audio": "audio", "video": "video",
        "terminal": "terminal", "tui": "tui", "game": "gaming",
        "sol": "solver", "code-server": "remote-dev", "ide": "ide",
        "openclaw": "openclaw", "nanobot": "agent", "hermes": "hermes-agent",
        ...
        "telemetry": "telemetry", "logging": "logging",
        ...
        "comfyui": "image-gen", "langfuse": "observability",
        "opik": "observability", "milvus": "vector-db",
        "txtai": "vector-search", "temporal": "workflow-engine",
        "dokku": "paas", "headscale": "vpn", "wireguard": "vpn",
        "netdata": "monitoring", "swag": "reverse-proxy",
        "open-webui": "chat-ui", "karpathy": "research",
        "deepresearch": "research", "autoresearch": "research",
        "awesome": "awesome-list", "agent-browser": "browser-automation",
        "vector": "vector-db", "embedding": "embeddings",
        "fine-tun": "fine-tuning", "training": "training",
        "rag": "rag", "multi-agent": "multi-agent", "team": "multi-agent",
    }
...
def detect_type(desc: str, fork: bool, readme: str = "") -> str:
    combined = ((desc or "") + " " + (readme or "")).lower()
    ...
    if "web app" in combined or "web-based" in combined:
        return "webapp"
    if "docker" in combined or "infrastructure" in combined:
...
def format_frontmatter(data: dict) -> str:
    """Format YAML frontmatter manually (no PyYAML dependency)."""
    lines = ["---"]
...
def generate_readme(owned: list, starred: list) -> str:
    ...
    for tag in sorted(combo_tags.keys())[:15]:
        ...
        readme += f"### `{tag}`\n- Owned: {', '.join(owned_names)}\n- Starred: {', '.join(starred_names)}\n\n"
...
def main():
    ...
    # Fetch owned repos (GraphQL schema: diskUsage, not size)
    ...
    # Fetch starred repos (REST schema: size, stargazers_count)
    ...
    for r in owned_data:
        ...
        entry = {
            "name": name, "short_name": name.split("/")[-1],
            "url": f"https://github.com/{name}",
            "description": desc or "No description",
            "type": type_, "status": status,
            "language": lang or "other", "size_kb": size,
            "stars": stars, "last_pushed": pushed,
            "license": license_info or "unknown", "tags": tags,
            "fork_of": parent if fork else None, "topics": topics,
        }
    ...
    for r in starred_filtered:
        ...
        for t in topics:
            t_lower = t.lower()
            tag_map = {"ai": "ai", "llm": "llm", "machine-learning": "ai",
                       "agent": "agent", "autonomous-agent": "agent",
                       "docker": "docker", "container": "docker",
                       "typescript": "typescript", "javascript": "javascript",
                       "react": "react", "go": "go", "golang": "go",
    ...
    for d in ["owned", "starred"]:
        expected = {safe_name(e["name"]) + ".md" for e in (owned_entries if d == "owned" else starred_entries)}
