...
Credential pools let you register **multiple API keys for the same provider**. When one key hits a rate limit or quota, Hermes auto-rotates to the next healthy key.
...
```bash
# Non-interactive (preferred — avoids shell history exposure):
...
**⚠️ Security:** Keys passed as CLI arguments may be captured in shell history. After adding a key via CLI, either:
- Rotate/regenerate the key from the provider dashboard, or
- Clear history: `history -c && history -w`

#### Rotation Strategies

Configure in `config.yaml` under `credential_pool_strategies`:
...
| Strategy | Behavior |
|---|---|
| `fill_first` (default) | Use first key until exhausted, then move to next |
| `round_robin` | Cycle through keys evenly — best for rate-limit avoidance |
...
|---|---|---|
| 429 Rate Limit | Retry same key once; second 429 rotates to next key | 1 hour |
| 402 Billing/Quota | Immediately rotate to next key | 24 hours |
| 401 Auth Expired | Try OAuth refresh first; rotate only if refresh fails | — |
| All keys exhausted | Fall through to `fallback_model` if configured | — |

#### Cross-Provider Fallback

When all pool keys for the primary provider are exhausted, Hermes can fall back to a different provider:

```bash
hermes config set fallback_providers '["openrouter"]'
# Or with a specific model:
hermes config set fallback_model 'openrouter/deepseek/deepseek-chat'
```

**Key distinction:** Pools = same-provider rotation (tried first). Fallback providers = cross-provider failover (only when all pool keys are exhausted).

#### When You Need Pools

- **API-key providers** (OpenRouter, Anthropic, OpenCode Zen): pools help dodge per-key rate limits
- **Nous Portal OAuth**: single auth covers 300+ models — pools usually unnecessary
- **Best practice**: combine pools (same-provider) with `fallback_providers` (cross-provider) for maximum resilience
...
- **Discord bot silently drops messages (no response, no error) — require_mention**: By default `discord.require_mention: true` — Hermes only responds to messages that @mention the bot. Messages without an @mention are silently ignored with no log entry. To make Hermes respond to all messages in allowed channels: `hermes config set discord.require_mention false` then restart the gateway (`systemctl --user restart hermes-gateway`). The `allowed_channels: ''` config already means "all channels unrestricted" — the `require_mention` gate is the more common filter.
- **Discord bot silently drops messages (no response, no error) — session lock**: A long-running non-ephemeral session can hold `_running_agents` and block new Discord messages. The `_busy_input_mode: "interrupt"` default silently drops messages that arrive while the session is locked. Use `/stop` in the Discord thread — it releases the running state lock (`release_running_state=True`). `/reset` does NOT release the lock. See `references/discord-session-lock.md` for full diagnosis and prevention (HERMES_AGENT_TIMEOUT config, allowed_channels addition).
...
| Session troubleshooting | See "Session File Permission Errors" and "Session Disk Bloat" above |
| OpenRouter credit issues | See "OpenRouter Credit Depletion (402 Errors)" above |
| Source code | `~/.hermes/hermes-agent/` |
| State migration | `references/state-migration.md` — dev/prod .db migration workflow |
| MiniMax API/endpoint quirks | `references/minimax-api-endpoints.md` — correct endpoints (api.minimax.io not .chat), vision `input_image` format, 401 diagnosis |
| MiniMax httpx 404 | `references/minimax-httpx-404-debug.md` — cron jobs 404 on MiniMax API, log absence diagnostic, HTML vs JSON body distinction |
| MiniMax failures | `references/minimax-api-failures.md` — model error 1211, empty response diagnosis |
| Cron job MiniMax 404 | `references/cron-minimax-provider-404.md` — `provider=custom` vs `provider=minimax` routing difference, cron-specific 404 on MiniMax API |
| Skill permission errors | `references/skill-permission-errors.md` — uid mismatch, skill not loading |
See `references/cloudflared-tunnel-watchdog.md` for watchdog script path, credential file locations, and systemd service fix.
See `references/host-file-transfer-ssh.md` for SSH pipe pattern for host→container file transfer.
| Cron host access auth | `references/cron-host-access-authenticated-tools.md` — authenticated CLI tools (gh) must run over SSH on host, not in container |

| Skill permission errors | `references/skill-permission-errors.md` — uid mismatch, skill not loading |
| Backup mechanism | `references/hermes-sync-backup.md` — cron-based backup script, paths, pitfalls (env sourcing, permission errors, wrapper gotchas) |
| Fun-Audio-Chat architecture | `references/fun-audio-chat-architecture.md` — server architecture, binary protocol, GPU requirements, integration patterns |
| Source code | `~/.hermes/hermes-agent/` |
