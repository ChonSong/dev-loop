...
## Data-Driven Generative Art (Personal Data Visualization)

When the source data is personal/user data (browsing history, location, habits, etc.):

### Data Pipeline
1. **Parse** raw data (JSON, CSV, SQLite) into structured records
2. **Categorize** using regex patterns on domain names or content tags
3. **Enrich** with derived metrics (visit frequency, recency, time-of-day patterns, session clustering)
4. **Encode** visual properties from data:
   - Position → time (radial: center=old, outer=new; or left-to-right: early→late)
   - Size → frequency/visit count (use `log(count)` not raw count — log scale prevents outliers from dominating)
   - Color → category (HSB: map category→hue, vary saturation/brightness for sub-categories)
   - Opacity → recency (recent = 1.0, oldest = 0.3)
   - Motion speed → activity level
5. **Exclude** sensitive domains via pattern list (medical, banking login, government services)

### Parallel Execution Pattern
When data acquisition is pending (e.g., file upload in progress):
- **Do NOT block.** Generate placeholder/mock data with realistic statistical properties
- Build all art code against placeholder data
- Swap placeholder → real data via `loadJSON()` or file replacement
- This lets the user see progress immediately and iterate on visuals while data transfers

### Complementary Tools
- **D3.js force graphs:** Use alongside p5.js for the interactive explorer version. p5.js for the art poster, D3 for the interactive atlas. Both consume the same `domains.json` + `d3-data.json`.
- **Pixel art** (`pixel-art` skill): Use `pixel_art.py` + `pixel_art_video.py` for retro variants. Generate treemap/shelf/spiral layouts in Python, render through Pillow.
- **HyperFrames** (`hyperframes` skill): Use for HTML→MP4 video export of animated compositions. GSAP timeline controls playback.
- **DESIGN.md** (`design-md` skill): Author a shared design token spec before coding. Ensures consistent colors/fonts across all output variants.

### Standard Output Matrix
For a complete deliverable set, generate 3 formats per art style:
- **Static poster:** 4K PNG (3840×2160), Puppeteer headless capture
- **Animation:** MP4 (30fps, 15-30s), CCapture.js or HyperFrames
- **Interactive:** HTML page with hover/explore, D3.js or p5.js
...
## Personal Data as Generative Art

When the user wants to visualize personal data (browsing history, location history, listening history, app usage, etc.) as generative art, use this extended pipeline. This is a **separate mode** — it requires a data pipeline before any art code.

### Phase A: Data Pipeline (Python, before any p5.js)

1. **Source identification** — Determine the source:
   - Google Takeout (`History.json` for browsing, `Location History.json`, YouTube, etc.)
   - Browser cookies (SQLite DB under `Default/Cookies` — only domains + last access, no titles)
   - API exports (Spotify, GitHub, Strava)
   - Local exports (CSV, JSON, SQLite)

2. **Parse & normalize** — Extraction script:
   ```python
   from urllib.parse import urlparse
   # For each record: domain = urlparse(url).netloc.replace("www.", "")
   # Normalize: ISO timestamp, hour, weekday, date fields
   ```

3. **Categorize** — Regex-based classification (priority order matters):
   - Social: twitter, x.com, facebook, instagram, reddit, discord, tiktok, linkedin, mastodon, bsky
   - Dev: github, gitlab, stackoverflow, npmjs, pypi, docker, localhost, vercel, netlify, heroku
   - News: bbc, cnn, nytimes, theguardian, reuters, bloomberg, ft.com, wsj
   - Shopping: amazon, ebay, etsy, aliexpress, shopify
   - Entertainment: youtube, netflix, spotify, twitch, steam, soundcloud, vimeo
   - Finance: coinbase, binance, robinhood, paypal, stripe, chase, wise, crypto
   - Education: coursera, edx, khanacademy, wikipedia, scholar.google, udemy
   - Travel: booking.com, airbnb, expedia, tripadvisor, kayak, skyscanner
   - Search: google.com/search, bing, duckduckgo, ecosia
   - Other: everything else

4. **Enrich** — For each domain/entity:
   - Compute visit frequency, first_seen, last_seen, hour distribution
   - Fetch favicon: `https://www.google.com/s2/favicons?domain=X&sz=64` → extract dominant color via Pillow
   - Session clustering: group visits with <30-min gap into sessions
   - Co-occurrence links: domains appearing in same session

5. **Output unified JSON** — All art scripts consume this:
   ```json
   {
     "nodes": [
       {"id": "github.com", "category": "dev", "count": 342, "color": "#24292e",
        "first": "2024-01-15", "titles": ["my-repo", "issue #42"]}
     ],
     "links": [
       {"source": "github.com", "target": "stackoverflow.com", "weight": 12}
     ],
     "meta": {
       "total_visits": 50000, "domain_count": 847,
       "date_range": ["2024-01-01", "2025-06-07"]
     }
   }
   ```

### Phase B: Art Generation (p5.js, consuming the JSON)

**Layout patterns for personal data:**

- **Temporal rings** — Center = oldest, edge = newest. Categories as angular segments. Radius = time. Node size = log(visit_count). Bezier connections between related domains.
- **Flow field particles** — Each visit = a particle. Curl noise drift. Category = color. Size = visit count. Trail fade.
- **Pixel mosaic** — Each domain = a tile. Bin-pack by visit count. Color from favicon. Grid with 1px gaps.
- **Force-directed graph** — D3.js. Domains = nodes. Co-occurrence = links. Cluster by category.

### Phase C: Multi-Style Planning

When producing multiple art styles from the same data:

1. **One shared DESIGN.md** — Single visual identity across all outputs (use `design-md` skill)
2. **Output matrix** — Each style × each format (static PNG + MP4 animation + interactive HTML)
3. **Build data pipeline once** — All styles consume the same enriched JSON
4. **Iterate on primary style first** — Get visual language right, then adapt
5. **Quality gate per output** — First-render excellence, structure, resolution, color, performance
...
- **Don't show everything** — Top 50-200 domains is enough. 5000+ = visual noise.
- **Don't forget seeded randomness** — `randomSeed()` + `noiseSeed()` always.
- **Don't ignore performance** — 100k+ visits needs pixel buffers, not individual `ellipse()`.
