#!/usr/bin/env python3
"""
Standalone Indeed job search via Apify API.
No dependencies beyond stdlib (urllib, json, os, sys, time, argparse).

Usage:
    python3 run_search.py --positions "Data Scientist,ML Engineer" \\
        --locations "Sydney, NSW" --country AU --max 200
"""
...
def main():
    parser = argparse.ArgumentParser(description='Search Indeed via Apify')
    ...
    parser.add_argument('--country', default='AU', help='Indeed country domain (AU, US, UK, etc)')
    ...
    print(f'Searching Indeed {args.country} for {positions} in {locations or "nationwide"}...')
    ...
    local_results = [j for j in items if j.get('country') == args.country]
    print(f'\nResults: {len(items)} total, {len(local_results)} in {args.country}')
    ...
    for job in items[:30]:
        ...
        print(f'  {title}{remote} | {company} | {location} | {salary}')
        print(f'    {url_job}')
    ...
    ts = int(time.time())
    outfile = os.path.join(outdir, f'{ts}.json')
