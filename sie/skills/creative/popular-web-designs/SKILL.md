...
the visual vocabulary. Correct pattern for repo redesigns: (1) load both skills,
(2) inspect the repo's actual source files to extract current tokens,
(3) create a token reference file in the repo (e.g. `themes/linear-tokens.ts`),
(4) write a delegate_task prompt including full current values, target values,
and design guidance, (5) verify type checks pass. Do NOT manually apply tokens
from this skill without following that workflow.
