...
## Variant Polish Complete ✅

**Commit**: `57aa225` — `fix(variants): polish edge cases`

### What was fixed:

| Task | File | Bug | Fix |
|------|------|-----|-----|
| **A. PLO4** | `plo4.py` | `_exact_equity` passed all 5 board cards to `evaluate_cards()` — it would pick 0, 1, or 2 hole cards, violating Omaha's "exactly 2 hole + 3 board" rule | Now iterates all C(5,3) board combos for each hole combo, finding best rank per player before comparing |
| **B. PLO5** | `plo5.py` | Same bug as PLO4 | Same fix (C(5,2) × C(5,3) = 100 combos properly enumerated) |
| **C. Omaha Hi/Lo** | `omaha_hi_lo.py` | `_is_valid_low` only checked rank ≤ 8 but allowed paired ranks (e.g., A-2-5-5-7 incorrectly qualified). Also `low_val` mapping was off by one (`ri+1` mapped "2" → low_val=1, same as Ace) | Added unique-rank check (5 distinct low values required). Fixed mapping: `ri+2` for non-Ace (2→2, ..., 8→8) |
| **D. Shortdeck** | `shortdeck.py` | Already correct — flush(7) > FH(6) > straight(5) ranking confirmed by tests | Verified all 30 Shortdeck tests pass |
| **E. Double Board** | `double_board.py` | `evaluate()` used module-level `random.randint()` with no reproducibility. Deck drawing mixed pop-at-random-index with random-shuffle patterns | Refactored to use `random.Random(seed)` instance. Boards are filled via `pop()` from a shuffled deck — guarantees zero card overlap between boards |
| **F. Bomb Pot** | `bomb_pot.py` | Missing `get_action_space()` method on `BombPotGameState`. No `FOLD` action type for post-flop | Added `get_action_space()` returning phase-appropriate actions. Added `FOLD` to `ActionType` enum. Updated `legal_actions()` to include fold post-flop |

### Test results: **115/115 passed** across all variant modules
