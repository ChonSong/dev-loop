...
## Phase 2 Equity Calculator — Completed

### What's Done

| Task | Status |
|------|--------|
| RangeSelector 13x13 grid component | ✅ Fixed via `ui-components` RangeGrid (proper CSS grid layout, suited/offsuit/pair color coding) |
| EquityChart with Recharts | ✅ Built at `apps/web/src/components/equity/EquityChart.tsx` (stacked bar chart, hero vs villain) |
| Monte Carlo equity engine (100k+ iterations) | ✅ `equity_vs_range()` supports configurable iterations up to 1M |
| Range expansion (JJ+, AKs, KJs → combos) | ✅ `RangeParser.get_all_combos()` expands all notations to specific suit combos |
| Equity vs range API | ✅ Full API at `/api/v1/equity/calculate` with GET/POST |
| Multi-way equity (3+ players) | ✅ `equity_vs_range_multiway()` supports multiple villain ranges |
| Omaha/Shortdeck equity | ⏸ Not started — requires 4-card hand evaluation |
| Redis caching | ✅ Cache key: `equity:{hero_hash}:{villain_hash}:{board_hash}:{iterations}`, 1hr TTL |
| Heatmap visualization | ✅ `/api/v1/equity/heatmap` returns all 169 preflop hands |
| EV calculation | ✅ `/api/v1/equity/ev` with bet sizing support |
| Playwright E2E tests | ✅ `apps/web/e2e/equity.spec.ts` with 8 test scenarios |

### Commits (this session)
```
5231d04 feat(poker-core): improve equity engine with Numba JIT and multi-way support
d8847fd test: add Playwright E2E tests for equity calculator
29dff79 feat: add equity API with Redis caching and multi-way support
b687bc0 feat: add EquityChart bar chart component
1e23ba4 feat: improve RangeGrid with proper 13x13 matrix layout
```

### Phase 3 Should Tackle

1. **Omaha/Shortdeck support** — 4-card evaluation, different hand rankings (Ace-to-5 low, etc.)
2. **Full equity page UI** — wire up RangeGrid, board input, results display to actual API
3. **Equity heatmap visual** — render heatmap data as a colored 13x13 matrix
4. **ICM calculator** — tournament chip EV calculations
5. **Training/Quiz mode** — present spots, track GTO deviation
6. **Solver integration** — connect to the gRPC solver service for on-demand GTO strategy
7. **Production deployment** — Neon DB setup, Vercel/Railway deployment
