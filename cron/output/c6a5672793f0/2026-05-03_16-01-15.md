...
## agent-os Phase 3 — PostgreSQL Schema Migrations

**NOTE**: As of commit b3495f94, no PostgreSQL setup exists yet. This phase creates the SQL schema migrations from SPEC.md.

**Load the `hermes-agent` skill first.**

### Context
- PostgreSQL is available at `postgresql://agentos:agentos_secure_pg_pass_2026@127.0.0.1:5432/agentos`
- Schema migrations should go in: `/home/sean/.hermes/agent-os/infra/migrations/`
- Use Python's `psycopg2` or `sqlalchemy` for migration scripts

### Steps

**Step 1 — Check if PostgreSQL is accessible**
```bash
ssh -o StrictHostKeyChecking=no sean@localhost "PGPASSWORD=agentos_secure_pg_pass_2026 psql -h 127.0.0.1 -U agentos -d agentos -c 'SELECT version();' 2>&1 | head -5"
```

**Step 2 — Create migrations directory**
```bash
ssh -o StrictHostKeyChecking=no sean@localhost "mkdir -p /home/sean/.hermes/agent-os/infra/migrations"
```

**Step 3 — Create initial migration script**
```bash
ssh -o StrictHostKeyChecking=no sean@localhost "cat > /home/sean/.hermes/agent-os/infra/migrations/001_initial_schema.sql << 'SQLEOF'
-- agent-os PostgreSQL Schema
-- Initial migration
...
```bash
ssh -o StrictHostKeyChecking=no sean@localhost "PGPASSWORD=agentos_secure_pg_pass_2026 psql -h 127.0.0.1 -U agentos -d agentos -f /home/sean/.hermes/agent-os/infra/migrations/001_initial_schema.sql 2>&1"
```

**Step 5 — Verify tables**
```bash
ssh -o StrictHostKeyChecking=no sean@localhost "PGPASSWORD=agentos_secure_pg_pass_2026 psql -h 127.0.0.1 -U agentos -d agentos -c '\\dt' 2>&1"
```
