# Cron Job: agent-os: Phase 2 — E2E verification (FAILED)

**Job ID:** d4994383fe5e
**Run Time:** 2026-05-03 14:00:38
**Schedule:** 0 14 * * *
...
The host's gh CLI is alr

[... output truncated ...]
```

## agent-os Phase 2 — E2E Verification

**NOTE**: The nanobot integration was confirmed working via MiniMax-M2.7 model. This phase verifies the end-to-end flow from dashboard to nanobot to MiniMax API.

**Load the `hermes-agent` skill first.**

### Context
- Dashboard backend: /home/sean/.hermes/agent-os/apps/dashboard/backend/src/index.ts (Express :3001)
- Nanobot: /home/sean/.hermes/agent-os/packages/nanobot/ (Python)
- MiniMax API base: https://api.minimax.chat
- MiniMax key: stored in /home/sean/.hermes/agent-os/.env as MINIMAX_API_KEY

### Steps

**Step 1 — Verify dashboard backend is running (or start it)**
```bash
ssh -o StrictHostKeyChecking=no sean@localhost "curl -sf http://localhost:3001/health 2>&1 || echo 'backend not running'"
```

**Step 2 — Verify nanobot is running (or start it)**
```bash
ssh -o StrictHostKeyChecking=no sean@localhost "curl -sf http://localhost:8001/health 2>&1 || echo 'nanobot not running'"
```

**Step 3 — Verify MiniMax API key is set**
```bash
ssh -o StrictHostKeyChecking=no sean@localhost "grep MINIMAX_API_KEY /home/sean/.hermes/agent-os/.env 2>&1 | grep -v PASTE || echo 'MINIMAX_API_KEY not configured'"
```

**Step 4 — If services not running, start them**
```bash
ssh -o StrictHostKeyChecking=no sean@localhost "cd /home/sean/.hermes/agent-os && docker compose up -d 2>&1 | tail -5"
```

**Step 5 — Test full E2E flow via nanobot**
```bash
ssh -o StrictHostKeyChecking=no sean@localhost "curl -s -X POST http://localhost:8001/aie -H 'Content-Type: application/json' -d '{\"prompt\":\"ping\",\"session_id\":\"test-$(date +%s)\"}' 2>&1 | head -50"
```

**Step 6 — Check AIE event log**
```bash
ssh -o StrictHostKeyChecking=no sean@localhost "ls -la ~/.nanobot/aie-logs/ 2>/dev/null && tail -5 ~/.nanobot/aie-logs/agent-events.jsonl 2>/dev/null || echo 'no aie log'"
```
...
**Dashboard backend**: running / not running
**Nanobot**: running / not running
**MiniMax API key**: configured / MISSING
**E2E test**: passed / failed / skipped
**AIE log**: entries found / empty / missing
**Status**: PASS / FAIL
```
