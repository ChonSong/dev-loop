...
18. **Always set `model` and `provider` explicitly on every cron job creation** — If you omit these or set `model: null`, the scheduler rejects the job with `400 - Model not exist`. Every cron job created without an explicit model silently fails at runtime. This is the single most common cause of cron failures.
...
19. **`cronjob action=run` does NOT execute immediately** — It queues for the next scheduler tick (~30-60 seconds). You will NOT see output in the current session. The job runs in a separate session and delivers via the configured channel. If `repeat` is near its limit (e.g. `"1/3"`), the scheduler silently drops the run — always set `repeat` to a fresh value (e.g. `"1/20"`) before triggering via `cronjob action=run`. If you need immediate execution with visible output, use `delegate_task` or direct terminal execution instead of cron.
...
See `references/hermes-web-computer-phase-engine.md` for a concrete example: the full 4-phase plan to complete hermes-web-computer v1.0 (Editor save, Agent chat, Voice UI, Browser tile, Dashboard migration, Polish).
See `references/hwc-phase-engine-may15-2026.md` for the current verified state of the HWC phase engine (paths, phase status, Go commands, cron job ID) — update this reference after each phase completes.
